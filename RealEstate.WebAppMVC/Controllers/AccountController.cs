...
namespace RealEstate.WebAppMVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private readonly OwnerRegisterCodeServices _ownerRegisterCodeManager;
        private IEmailService EmailManager { get; set; }
        ...
        [Inject]
        public AccountController(IEmailService emailService, OwnerRegisterCodeServices ownerRegisterCodeServices)
        {
            EmailManager = emailService;
            _ownerRegisterCodeManager = ownerRegisterCodeServices;
        }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (Request.Url != null)
                    {
                        ...
                        await EmailManager.SendEmailAsync(user.Email, "Confirm your account", "Please confirm your account by clicking <a href=\"" + callbackUrl + "\">here</a>");
                    }
        ...
        public async Task<ActionResult> RegisterAgent(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await UserManager.AddToRoleAsync(currentUser.Id, "Agent");
            ...
            return Json("STATUS_FAIL", JsonRequestBehavior.AllowGet);
        }
        ...
        public async Task<ActionResult> RegisterOwner(RegisterOwnerViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (_ownerRegisterCodeManager.CheckOwnerRegisterCode(model.RegisterCode))
                {
                    var owner = new OwnerUsers { UserName = model.UserName, Email = model.Email };
                    var result = await UserManager.CreateAsync(owner, model.Password);
                    if (result.Succeeded)
                    {
                        var currentUser = await UserManager.FindByNameAsync(owner.UserName);
                        ...
                        if (Request.Url != null)
                        {
                            ...
                            await EmailManager.SendEmailAsync(owner.Email, "Confirm your account", "Please confirm your account by clicking <a href=\"" + callbackUrl + "\">here</a>");
                        }
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new ClientUsers { UserName = model.Email, Email = model.Email };
        ...
        {
            var code = _ownerRegisterCodeManager.AddOwnerRegisterCode();
        ...
        [HttpPost]
        public async Task<ActionResult> CreateOwner(OwnerCreateViewMode model)
        {
            if (!ModelState.IsValid)
            {
                return Json("STATUS_ERROR");
            }
        ...
        [HttpPost]
        public ActionResult CreateOwnerMock()
        {
            return Json(new UsersIdInfoViewModel
            {
                Id = "1",
